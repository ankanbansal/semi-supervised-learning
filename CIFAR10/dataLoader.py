...
class RegularizedWeightedBatchSampler(object):
    def __init__(self, dataset_size, num_sup, num_unsup, batch_size, num_transformations,
            sup_to_tot_ratio, sup_indices=None):
        self.all_indices = list(range(dataset_size))
        if sup_indices:
            self.sup_indices = np.array(sup_indices)
        else:
            self.sup_indices = np.random.choice(self.all_indices, size=num_sup, replace=False)
        self.unsup_indices = list(set(self.all_indices) - set(self.sup_indices))[:num_unsup]
        self.batch_size = batch_size
...
def regularized_weighted_loaders(options):
    ...
    if options['sup_indices_file']:
        sup_indices = json.load(open(options['sup_indices_file']))
    else:
        sup_indices = None
    ...
    sampler = RegularizedWeightedBatchSampler(train_dataset.train_data.shape[0], options['num_sup'],
            options['num_unsup'], options['batch_size'], options['num_transformations'],
            options['sup_to_tot_ratio'], sup_indices=sup_indices)
    json.dump(list(sampler.sup_indices),open(options['temp_file'],'w'))
